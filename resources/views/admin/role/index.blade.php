		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			<thead>
				<tr class="text-c">
					...
					<th width="80">角色名称</th>
					<th width="80">对应权限</th>
					<th width="80">控制器方法</th>
					<th width="120">加入时间</th>
					<th width="75">是否已启用</th>
					<th width="120">操作</th>
				</tr>
			</thead>
			<tbody>
			@foreach($data as $v)
				<tr class="text-c">
					<td><input type="checkbox" value="" name="role_name"></td>
					<td>{{$v->id}}</td>
					<td class="text-l">{{$v->role_name}}</td>
					<td>{{$v->priv_ids}}</td>
					<td>{{$v->priv_ac}}i</td>
					<td>2014-6-11 11:11:42</td>
					<td class="td-status"><span class="label label-success radius">已启用</span></td>
					<td class="f-14 td-manage"><a style="text-decoration:none" onClick="article_stop(this,'10001')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a> <a style="text-decoration:none" class="ml-5" onClick="role_edit({{$v->id}})" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="article_del(this,'10001')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
			@endforeach
			</tbody>
		</table>
...
function role_edit(id){
    layer_show('修改角色','{{url("admin/role/update")}}/'+id,1000);
}
