						...
						@foreach($priB as $v1)
							@if($v1->parent_id==$v->id)
								<dl class="cl permission-list2">
									<dt>
										<label class="">
											<input type="checkbox" value="{{$v1->id}}" name="priv[]" id="user-Character-0-0">
											{{$v1->priv_name}}</label>
									</dt>
									@foreach($priC as $v2)
										...
										<dt>
											<label class="">
												<input type="checkbox" value="{{$v2->id}}" name="priv[]" id="user-Character-0-0-0">
												{{$v2->priv_name}}</label>
										</dt>
										@endif
									@endforeach
...
$(function(){
    $('#form-admin-role-add').submit(function (event) {
		event.preventDefault();
		var data = $(this).serialize();
		if($('input:checked').length < 1){
			layer.msg('至少选一个吧',{icon:5,time:2000});
			return false;
		}
		$.ajax({
			type:'post',
